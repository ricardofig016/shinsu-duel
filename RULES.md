...
3. A unit deployed in a position can use an ability only if the combat slot for that position is available. A [Free](#keywords) ability is the exception: it doesn't require the slot to be available. A spent slot doesn't stop the player from playing another unit in that position.
