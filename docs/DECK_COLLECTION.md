...
Decks may be **saved with rule violations** (they are flagged with per-violation problems and stay out of non-dev games), but never with unbuildable card lists: no room type can start a game with them. Legality is recomputed on every read, because the compiled catalog can change between saves. The deck-to-engine boundary lives in `server/utils/card-catalog.js` (`buildSlugIndex`, the single slug → card conversion point): the gateway resolves the stored slugs to runtime cardIds with one index per deck when a game is created, and re-checks buildability and legality there, so a deck edited or deleted between pick and start cannot reach the engine.
...
`server/data/starter-decks.json` holds the curated templates, authored **by card slug**; `server/decks/starterDecks.js` loads them lazily, so importing the server never reads the shipped deck data. `StarterDeckAudit` validates every template against the shipped catalog: a slug that no longer resolves (as after a card rename), a card that became Unreachable, or a deck that breaks a rule fails that suite. `server/decks/deckProvisioning.js` copies every template into a new account inside `createUser` (`server/routes/auth.js`), once, at creation: there is no lazy or legacy provisioning, so a user who deletes every deck keeps an empty collection. A failure part-way through provisioning removes the copies already created. Copies are ordinary decks.
...
**Deck list** — a table with one row per deck: a fan of the deck's three most expensive distinct units (cheapest left, most expensive right and frontmost, rendered as the shared card component; right-click opens the card detail overlay ([CARD_VERTICAL_COMPONENT.md](./CARD_VERTICAL_COMPONENT.md#card-detail-overlay)), name, size, composition (units / skills / equipment), average card cost, legality flag (hover shows the problems), last update, and Edit / Duplicate / Delete actions. Clicking the row opens the deck. Filters: name search, legality (all / legal / not legal), sorting (name and average card cost, both directions), and a "containing cards" filter — text, type, and affiliation criteria applied to the deck's contents where each active facet is satisfied independently by some card in the deck.
