...
| `slay`             | `unit:killed`                               | Emitted when a unit's HP reaches 0, after death-intent and Undying interception. Distinct from `unit:destroyed`. Matches when `payload.sourceId` or `payload.slayerId` equals the unit.      |
...
| `has_all_equipped` | `equipment:attached` / `equipment:detached` | Fires when the unit is equipped with every card in `cardNames[]`; re-checked on attach and detach.                                                                                           |
