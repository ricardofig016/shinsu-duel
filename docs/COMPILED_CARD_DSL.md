...
| `modify_ability`   | `target`, `effect`, `position?`, `if?`                         | `target` selects whose abilities are augmented; when such a unit's ability targets the side in `effect.target`, also resolve `effect` against that target.                                                                                                             |
