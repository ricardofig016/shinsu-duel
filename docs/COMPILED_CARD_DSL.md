...
| `copy_traits`        | `target`, `source`                         | Copy traits from `source` onto `target`. A triggered resolution may thread `sourceTraits` (a `{ trait, value?, disabled? }` snapshot) to copy traits the source no longer holds. |
...
`side: enemy` with `count > 1` (no `scope`, or `scope: single`) selects N enemy units and is normalized to the `enemies` descriptor: targetable Taunt units are locked as mandatory and the player chooses only the remaining free (non-Taunt) slots. When the Taunt units already satisfy `count`, or when every remaining candidate is forced, the whole selection is committed as locked and the player only confirms it. When there are more Taunt units than `count`, the player chooses `count` among them. `side: enemy` with no count (or `count: 1`) is the single-target `enemy` descriptor and is Taunt-collapsed. `random: true` selects targets automatically via the seeded RNG (no decision); a `Blinded` source is treated the same way, with Taunt still enforced for enemy targets.
...
`equip`, `slay`, `deploy`, `given`, `kill`, `ally_dies`, `enemy_dies`, `damaged_by`, `round_start`, `round_end`, `deal_damage`, `ability_used`, `summon`, `draw`, `reclaim`, `free_ability_played`, `quick_ability_used`, `activation`, `skill_played`, `skills_played`, `dies`, `evolve`, `has_all_equipped`, `equipment_ignited`, `silenced`.

`deal_damage` accepts an optional `amount` threshold ("deals 7+ damage to a single target"): it fires only when one damage application against a single target reaches `amount` after modifiers. The threshold reads the hit's full post-modifier amount (`hitAmount` on the `unit:damage:applied` payload, before the target's remaining HP clamps it), so overkill counts; payloads without `hitAmount` fall back to the applied `amount`. Condition ticks (Burned, Cursed) are not the bearer's damage applications and never fire it. `equipment_ignited` fires for the unit whose attached equipment ignited. `silenced` carries two side descriptors — `target: { side }` (who was silenced) and `source: { side }` (whose action caused it, per the `sourceOwner` on the `unit:silenced` payload) — both relative to the passive owner; an omitted `source` matches any cause, and an unattributed silence matches neither `ally` nor `enemy`. A `silenced`-triggered `copy_traits` receives the silenced unit as its source plus the silenced-moment trait snapshot (`sourceTraits`): the removed traits no longer exist on the stack when the effect resolves.

`cardType` (`unit` | `skill` | `equipment`) further filters `draw` / `equip` / `reclaim` triggers to a card type. `dies` is a unit's own death; `ally_dies` and `enemy_dies` are an ally's / enemy's death respectively (optionally filtered by `rank`). An `ally_dies` trigger includes the source unit's own death per the ally definition, and passive death triggers fire on `unit:killed` — the lethal pipeline announces the death while the dying unit's own passives are still subscribed, so a self-`dies` passive resolves. Non-death removals (substitution, landmark replacement, returning to hand) never fire them. `draw` fires for any card draw, including the round-start draw; `reclaim` fires for effect-driven reclaims; `free_ability_played` fires when an ability with Free is used. `evolve` fires on the `unit:evolving` announcement that precedes an evolution and never on a `transform`-effect revert; `has_all_equipped` carries `cardNames[]` and fires when the unit is equipped with every listed card. `skills_played` carries `count` and is transformation-only: it fires on `skill:applied` once its bearer's owner has played at least `count` skills this game (the counter is game-long and cumulative; the owner's own plays only). `activation` fires on the `unit:activation` event and only for the unit that event names (`payload.unitId`), so an `activate` effect replays the target's passives without touching other copies of the card.

Transformation triggers author these same structured trigger objects: a unit's `evolve:` list and an equipment's `ignition:` list each hold trigger entries with display `raw` text (required there, since the trigger is the whole entry). The compiler validates each `type` against the catalog and resolves the `<root> <next stage>` / `<name> - Ignited` cross-reference.
