...
Multi-target enemy selection goes through `TargetResolver.resolveTargetSelection()`, which returns either a random auto-selection (`{ auto: true, ids }`) or a decision plan (`{ auto: false, lockedIds, freeCandidates, freeCount }`). `auto` is reserved for `random`/Blinded picks, which the seeded RNG resolves with no decision. Every other outcome defers to the player: mandatory Taunt units are committed as `lockedIds` and only the free slots are offered as choices. When the Taunt units already satisfy the count, or when every remaining candidate is forced, the plan commits the whole selection as locked with zero free slots, so the effect resolves only after the player confirms it. When there are more targetable Taunt units than slots, the player chooses `count` among them. The same planner drives every choice descriptor (`enemy`, `enemies`, `enemy_frontline`, `enemy_backline`, `unit`, `ally`), so line-scoped and any-unit effects enforce Taunt exactly like plain `enemy` targeting.
...
A target decision is created for **every** player-facing target selection, including fully forced ones: the engine commits the forced targets as `lockedIds`, presents them pre-selected, and pauses with `minChoices: 0` until the player confirms. `lockedIds` carries targets that are already committed (mandatory Taunt units, or the whole selection when nothing is left to choose) so clients can render them as pre-selected and disabled; the player's `choices` cover only the remaining `minChoices..maxChoices` free slots — `0..0` is a pure confirmation. Only random selections (an explicit `random` target or a `Blinded` source) resolve without any decision. Choices are validated against the free slots only: a submitted choice may never reference a `lockedIds` target.
