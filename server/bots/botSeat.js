 ...
 *
 * The seat is named from the roster, behaves through the spec's playstyle,
 * fields decks through the spec's deck method, and answers moves through a
 * controller bound to the session registry and the gateway's validated
 * paths. Its rng is the room seed's bot derivation, so the seat plays the
 * same game a replay would.
 *
 * @param {object} args
 * @param {string} args.roomCode the room this seat plays in
 * @param {object} args.spec a validated bot spec from `parseBotSpec`
...
export function createBotSeat({ roomCode, spec, seed, opponentName, registry, submitter }) {
  ...
  const bot = getBot(spec.bot);
  const rng = new SeededRng(deriveBotSeed(seed));
  return {
    botId: bot.id,
    seatName: bot.seatName,
    deckMethodId: spec.deckMethod,
    deckMethod: botDeckMethods.get(spec.deckMethod),
    opponentName,
    rng,
    controller: new BotSeatController({
      roomCode,
      seatName: bot.seatName,
      playstyle: botPlaystyles.get(spec.bot),
      rng,
      registry,
      submitter,
    }),
