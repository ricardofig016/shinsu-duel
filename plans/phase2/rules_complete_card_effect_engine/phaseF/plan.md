...
3. **Resolved (2026-08-23)**: `modify_ability` augments apply **once per target per trigger**. `UseAbilityAction` resets the augment dedupe set before each `modify_repeat` trigger, so a repeated ability re-applies its augments once per trigger while a multi-step ability within one trigger still applies each augment once.
